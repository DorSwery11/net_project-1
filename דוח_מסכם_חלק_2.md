...
**מגישים:** [שם סטודנט 1] [ת.ז.] ו[שם סטודנט 2] [ת.ז.]  
**תאריך:** [תאריך נוכחי]  
**קישור ל-GitHub:** [קישור לפרויקט]
...
- פקודה `send <name> <message>` → הודעה פרטית ללקוח ספציפי
...
לדוגמה:
```bash
python chat_client_gui.py Alice
python chat_client_gui.py Bob
```
...
**צד שמאל - השרת (Terminal):**
```
Server started on localhost:9999
Waiting for clients to connect...
New connection from ('127.0.0.1', 54321)
Client 'Alice' registered from ('127.0.0.1', 54321)
New connection from ('127.0.0.1', 54322)
Client 'Bob' registered from ('127.0.0.1', 54322)
Client 'Alice' disconnected
Client 'Bob' disconnected
```

**צד ימין - הלקוח הטקסטואלי:**
```
Connected to server as 'Alice'

==================================================
Simple Chat - Broadcast or direct messages

Available Commands:
  <message>               - Send to everyone (broadcast)
  send <name> <message>   - Send direct message to a client
...
**2. הודעה פרטית:**
- **קלט:** המשתמש מקליד `send Bob Hello` ב-CLI, או ממלא Target="Bob" ו-message="Hello" ב-GUI
- **עיבוד:** הלקוח שולח `CHAT:Bob:Hello` לשרת, השרת מעביר רק ל-Bob
- **פלט:** ההודעה מופיעה רק אצל Bob (ולא אצל אחרים)
...
|-----|------------|
| Frame | 42 |
| גודל החבילה | 65 bytes (520 bits) |
| Source Port | 54321 (הלקוח) |
| Destination Port | 9999 (השרת) |
| Sequence Number | 15 |
| Acknowledgment | 28 |
...
תמשנו ב-Cursor AI (Claude) במהלך הפיתוח:
...
- "Design a simple GUI for the chat client with a message input field and green Send button"
- "Implement both broadcast and targeted messages between clients"
- "Explain TCP flags and what PSH,ACK means in Wireshark"
- "How to prevent race conditions when multiple threads access the same dictionary?"
- "Update the chat to meet the requirements: server should open chat between two clients by name"

כל הקוד נבדק והובן על ידינו לפני השימוש, והותאם לדרישות הפרויקט.
...
**קבצי קוד:**
- `chat_server.py` - שרת TCP עם תמיכה ב-multi-threading
- `chat_client.py` - לקוח טקסטואלי (CLI)
- `chat_client_gui.py` - לקוח עם ממשק גרפי (GUI)

**קבצי לכידה:**
- `chat_traffic.pcap` - תעבורת הצ'אט מ-Wireshark

**צילומי מסך:**
- [Screenshot_Server.png] - השרת בזמן פעולה
- [Screenshot_Client_CLI.png] - הלקוח הטקסטואלי
- [Screenshot_Client_GUI.png] - הלקוח הגרפי
- [Screenshot_Wireshark.png] - תעבורה ב-Wireshark

**תיעוד:**
- `README_chat.md` - הוראות שימוש באנגלית
- `הדרכה_שימוש.md` - הוראות שימוש בעברית
- `דוח_מסכם_חלק_2.md` - דוח זה

---

**[שם סטודנט 1] ו[שם סטודנט 2]**  
קורס רשתות תקשורת, [חודש ושנה]
